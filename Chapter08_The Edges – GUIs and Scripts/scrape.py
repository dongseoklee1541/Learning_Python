# -*- coding: utf-8 -*-
import argparse
import base64 # base 64 to save the images within JSON
import json
import os
...
def _matches_extension(filename, extension_list):
 """ 코드의 간결함을 위해서, 연습을 위해 다 모으고 필요한것만 잘라내는 방법으로 선택"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser( #argparse useing
        description='Scrape a webpage.')
    parser.add_argument( 
        '-t',
        '--type', # also allowing long version
        choices=['all', 'png', 'jpg'],
        default='all',
    ...
    args = parser.parse_args() # in order to parse all args.
